...
#define BOARD_BCIFACE_PROVIDER_UART1    1
#define BOARD_BCIFACE_PROVIDER_USB      2
...
/**
 * @name Board Management Functions
 * 
 * These functions should be implemented somewhere, so that they are 
...
static inline void board_iface_USBCDC_txdhandler(uint8_t intfNum);
/**@}*/ 

extern BYTEBUF_SIZE_t bcif_usb_active_chunksize;
extern uint8_t bcif_handle_txdone;
extern uint8_t bcif_tx_inprogress;

static inline void board_iface_USBCDC_txdhandler(uint8_t intfNum){
    if (intfNum == BOARD_BCIFACE_USB_INTF){
        bcif_handle_txdone = 1;
